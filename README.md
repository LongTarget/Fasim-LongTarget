...
LongTarget was developed to predict one or many lncRNAâ€™s DNA binding motifs and binding sites in one or many genome regions based on all known ncRNA/DNA base pairing rules (Hoogsteen and reverse Hoogsteen base pairing rules). LongTarget consists of a few C/C++ programs, and is distributed under the AGPLv3 license. It can be used as a standalone program, and we have also integrated it into the lncRNA database LongMan at the website http://lncRNA.smu.edu.cn, making it a web service. Our use of LongTarget indicates that it can satisfactorily predict lncRNAsâ€?DNA binding motifs and binding sites, and multiple pipelines have been developed to seamless bridge database search and lncRNA/DNA binding prediction.
...
- [sswNew.cpp](./sswNew.cpp):The Program of getting the maximal score of each column,the maximal scores that are >Threshold
- [ssw.h](./ssw.h): the header file of sswNew.cpp
- [ssw_cpp.cpp](./ssw_cpp.cpp):The program of getting the maximal scores that are local maximum
- [ssw_cpp.h](./ssw_cpp.h):the header file of ssw_cpp.cpp
- [fastsim.h](./fastsim.h):The fasim program of getting k non-overlapping local alignments.
...
```
./fasim -f1 testDNA.fa -f2 H19.fa -O /home/test/example -lg 40 
```

A more complex case is:

```
./fasim -f testDNA.fa -s H19.fa -O /home/test/example -c 6000 -i 70 -S 1.0 -nt 25 -na 1000 -pc 1 -pt -500 -ds 10 -lg 60 -F false
```
...
F	doFastSim			if true, fastSIM function will be used instead of SIM function.
```
